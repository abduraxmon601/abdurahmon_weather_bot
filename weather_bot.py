...
# To'liq URL manzilini ko'rsatish kerak
...
async def get_weather_data(city: str) -> Dict[str, Any]:
    try:
        ...
        # requests sinxron kutubxona, lekin kichik loyihalar uchun yetarli
...
def format_weather_message(data: Dict[str, Any], city: str) -> str:
    ...
    if temp < 0: emoji = "❄️"
    elif temp < 10: emoji = "☁️"
    elif temp < 20: emoji = "⛅️"
    elif temp < 30: emoji = "☀️"
    else: emoji = "🔥"

    return f"""
{emoji} <b>{data['name']}</b> shahri ob-havosi {emoji}
...
tushinish uchun /help buyrugini berng🤔"""
